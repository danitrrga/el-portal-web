...
| **RESP-05** — `body` + all 8 page roots on `vh`→viewport-relative cascade | `src/app/globals.css` body rule + `.min-h-viewport` `@utility` (both `@supports`-gated, 05-01). `grep -rln "min-h-viewport" src/app/page.tsx src/app/*/page.tsx` → all 8 route files (05-04). **Amended 2026-07-31 (code review WR-02):** the unit is now `dvh`, not `svh`. A `min-height` is a floor and cannot clip, so `svh` (the *smallest* viewport height) only ever placed the floor below the visual viewport when the mobile URL bar retracted; `dvh` tracks the visual viewport in both directions. The `100vh` fallback and the `@supports` wrapper (minifier-safety) are unchanged |
